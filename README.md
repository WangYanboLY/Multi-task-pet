...
桌宠观察到以前未记录的回答完成标记后，会尝试发送 macOS 本地通知，并让黄色数字增加。首次建立观察基线时不会把历史回答当作新提醒；通知需要系统授权。点击通知或在桌宠里成功打开 Codex、Claude 桌面 Code 或网页对话，会把该回答标为已读；没有可打开入口的记录可手动点“标为已读”。桌宠不能可靠地读取 Codex、Claude 和浏览器之间统一的“是否已经在原应用看过”状态；从其他应用直接打开对话不会自动清除黄色数字。
