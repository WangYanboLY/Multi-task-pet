...
面板顶部可在 `GPT`、`Claude` 两个来源间切换。`GPT` 汇集 Codex 与 ChatGPT；`Claude` 汇集 Claude 网页、Claude 桌面 Code 和 Claude Code CLI。选定来源后，再选择“活跃对话”或“需要处理”。任务族卡片默认折叠，点击卡片可展开；有链接的任务点击标题或内容区可打开对话。每条任务旁有“已处理”复选框，可把已经处理过的任务从当前列表移走。首次打开时默认选择运行中对话最多的来源，数量相同则选择 `GPT`；手动切换后保留所选来源。
...
桌宠观察到以前未记录的回答完成标记后，会尝试发送 macOS 本地通知，并让黄色数字增加。首次建立观察基线时不会把历史回答当作新提醒。GPT 和 Claude 的通知各自需要一次系统授权；授权提示会显示为「Agent Pet · GPT」或「Agent Pet · Claude」。点击通知或在桌宠里成功打开 Codex、Claude 桌面 Code 或网页对话，会把该回答标为已读；没有可打开入口的记录可手动点“标为已读”。桌宠不能可靠地读取 Codex、Claude 和浏览器之间统一的“是否已经在原应用看过”状态；从其他应用直接打开对话不会自动清除黄色数字。
