...
面板顶部可在 `GPT`、`Claude` 两个来源间切换。`GPT` 汇集 Codex 与 ChatGPT；`Claude` 汇集 Claude 网页、Claude 桌面 Code 和 Claude Code CLI。选定来源后，再选择“活跃对话”或“需要处理”。“活跃项目”直接列出各项目；每个项目自身可展开，折叠时只显示项目名称，点击后才显示具体任务。有链接的任务点击标题或内容区可打开对话。每条任务旁有“已处理”复选框，可把已经处理过的任务从当前列表移走。首次打开时默认选择运行中对话最多的来源，数量相同则选择 `GPT`；手动切换后保留所选来源。
